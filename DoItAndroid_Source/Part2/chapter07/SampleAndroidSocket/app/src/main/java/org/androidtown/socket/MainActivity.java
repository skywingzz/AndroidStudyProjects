package org.androidtown.socket;

import android.os.Bundle;
import android.os.Handler;
import android.os.Message;
import android.support.v7.app.AppCompatActivity;
import android.util.Log;
import android.view.View;
import android.widget.Button;
import android.widget.EditText;
import android.widget.TextView;

import java.io.DataInput;
import java.io.DataInputStream;
import java.io.DataOutputStream;
import java.io.IOException;
import java.io.ObjectInputStream;
import java.io.ObjectOutputStream;
import java.net.Socket;

/**
 * 안드로이드에서 소켓 클라이언트로 연결하는 방법에 대해 알 수 있습니다.
 *
 * @author Mike
 *
 */
public class MainActivity extends AppCompatActivity {

    DataOutputStream outstream;
    DataInputStream instream;

    EditText input01, msg;
    ChatHandler chatHandler;
    TextView textView;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_main);

        input01 = (EditText) findViewById(R.id.input01);
        chatHandler = new ChatHandler();
        textView = (TextView) findViewById(R.id.text01);
        msg = (EditText) findViewById(R.id.msg);

        // 버튼 이벤트 처리
        Button button01 = (Button) findViewById(R.id.button01);
        button01.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                String addr = input01.getText().toString().trim();

                ConnectThread thread = new ConnectThread(addr);
                thread.start();
            }
        });

        Button send = (Button) findViewById(R.id.send);
        send.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                sendMsg(new View(MainActivity.this));
            }
        });

    }

    String chatName = "[guest]";
    boolean flag = true;
    public void sendMsg(View view) {
        if(flag) {
            String chatMsg = chatName + "님이 접속하셨습니다.";
            try {
                outstream.writeUTF(chatMsg);
                Message message = chatHandler.obtainMessage();
                message.obj = chatMsg;

            } catch (IOException e) {
                e.printStackTrace();
            }
            flag = false;
        } else {
            try {
                String chatMsg = chatName + msg.getText().toString();
                outstream.writeUTF(chatMsg);
                Message message = chatHandler.obtainMessage();
                message.obj = chatMsg;
            } catch( IOException e) {
                e.printStackTrace();
            }
        }
    }

    /**
     * 소켓 연결할 스레드 정의
     */
    class ConnectThread extends Thread {
        String hostname;

        public ConnectThread(String addr) {
            hostname = addr;
        }

        public void run() {

            try {
                int port = 5001;

                Socket sock = new Socket(hostname, port);
                outstream = new DataOutputStream(sock.getOutputStream());
                instream = new DataInputStream(sock.getInputStream());
                ClientThread thread = new ClientThread();
                thread.start();

                Message message = chatHandler.obtainMessage();
                message.obj = hostname + "에 접속되었습니다";

            } catch(Exception ex) {
                ex.printStackTrace();
            }

        }
    }

    class ClientThread extends Thread {

        @Override
        public void run() {
            String obj = null;
            try {
                obj = instream.readUTF();

                Message msg = chatHandler.obtainMessage();
                msg.obj = obj;
                chatHandler.sendMessage(msg);
                Log.d("MainActivity", "서버에서 받은 메시지 : " + obj);
            } catch(Exception e) {

            }


        }
    }

    class ChatHandler extends Handler {
        public void handleMessage(Message msg) {
            textView.append("\n" + msg.obj.toString());
        }
    }

}
